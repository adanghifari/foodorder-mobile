...
class _MenuPageState extends State<MenuPage> {
  ...
  // Fungsi untuk mendapatkan menu berdasarkan kategori yang aktif
      ...
      child: Column(
        ...
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: ['Hidangan', 'Cemilan', 'Minuman']
                  .map((cat) => _buildTabItem(cat))
                  .toList(),
            ),
          ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ClipRRect(
            borderRadius: BorderRadius.circular(15),
            child: Image.asset( // Menggunakan Image.asset karena file lokal
              menu['img'], 
              width: 90, 
              height: 90, 
              fit: BoxFit.cover,
              errorBuilder: (context, error, stackTrace) => 
                Container(width: 90, height: 90, color: Colors.grey[300], child: const Icon(Icons.broken_image)),
            ),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(menu['desc'], maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.grey, fontSize: 12)),
                const SizedBox(height: 8),
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('Rp ${menu['price_raw']}', style: const TextStyle(fontWeight: FontWeight.w900, color: Color(0xFFC8641E))),
                    IconButton.filled(
                      onPressed: _tambahKeKeranjang,
                      icon: const Icon(Icons.add),
                      style: IconButton.styleFrom(backgroundColor: const Color(0xFFC8641E), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                    )
                  ],
                )
              ],
  ...
  Widget _buildCartButton() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(20),
        boxShadow: [BoxShadow(color: const Color(0xFFC8641E).withAlpha(102), blurRadius: 20, offset: const Offset(0, 10))],
      ),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          const Row(
            children: [
              ...
              SizedBox(width: 10),
              Text('Lihat Keranjang', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
            ],
          ),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
            decoration: BoxDecoration(color: Colors.white.withAlpha(51), borderRadius: BorderRadius.circular(10)),
            child: Text('$cartCount Item', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          )
        ],
      ),
    );
